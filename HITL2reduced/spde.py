...
class spde:
    def __init__(self, model=2, reduce=False, method=1):
        ...
        self.Q = sparse.csc_matrix(
            (np.array(tmp[:, 2], dtype="float32"), (tmp[:, 0].astype('int32'), tmp[:, 1].astype('int32'))),
            shape=(self.n, self.n))
    ...
    def sample(self, n=1):
        ...
        if self.method == 2:
            ...
            data = data[:self.n, :] + self.mu3.reshape(-1, 1) + np.random.normal(size=self.n * n).reshape(self.n, n) * \
                   self.sigma[1]
        else:
            z = np.random.normal(size=self.n * n).reshape(self.n, n)
            data = self.Q_fac.apply_Pt(self.Q_fac.solve_Lt(z, use_LDLt_decomposition=False)) + np.random.normal(
                size=self.n) * self.sigma[1] + self.mu3
        return (data)

...
class PathPlanner:
    ...
    def load_waypoint(self):
        self.coordinates_waypoint = pd.read_csv(FILEPATH + "Simulation/PreConfig/WaypointGraph/WaypointGraph.csv").to_numpy()
    ...
    def setup_knowledge(self):
        ...
        self.ind_sample_grid = get_ind_at_location3d_xyz(self.coordinates_spde_xyz,
                                                         self.knowledge.current_location.X_START,
                                                         self.knowledge.current_location.Y_START,
                                                         self.knowledge.current_location.Z_START)
    ...
    def run(self):
        for i in range(self.num_steps):
            ...
            print("next location: ", self.next_location.X_START, self.next_location.Y_START, self.next_location.Z_START)
            self.ind_sample_grid = get_ind_at_location3d_xyz(self.coordinates_spde_xyz, self.next_location.X_START,
                                                             self.next_location.Y_START, self.next_location.Z_START)

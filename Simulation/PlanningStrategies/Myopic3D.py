...
class MyopicPlanning3D:
    ...
    def find_next_waypoint(self):
        ...
        for k in range(len(self.knowledge.ind_neighbour_filtered)):
            eibv.append(self.get_eibv_1d(self.knowledge.ind_neighbour_filtered[k]))
        t2 = time.time()
        if len(eibv) == 0:  # in case it is in the corner and not found any valid candidate locations
            ...
            ind_next = self.knowledge.ind_neighbour_filtered[ind_minimum_eibv]
            self.knowledge.next_location = self.get_location_from_ind(ind_next)
    ...
    def filter_neighbouring_loc(self):
        ...
        for i in range(len(self.ind_neighbour_locations)):
            ...
            if ind_candidate_location != self.knowledge.ind_current_location:
                vec2 = self.get_vector_between_locations(self.knowledge.current_location,
                                                         self.get_location_from_ind(ind_candidate_location))
